...
def _spec_list() -> list[FieldSpec]:
    ...
    return [
        ...
        FieldSpec("SEARCH_BREAKER_ENABLED", "bool", "慢站熔断",
                  hint="连不通的站会吃满整个超时预算，而聚合搜索要等最慢的那个"),
        FieldSpec("SEARCH_BREAKER_THRESHOLD", "int", "熔断阈值", minimum=1, maximum=20,
                  hint="连续多少次「吃满预算且零结果」才冷却；有结果的慢站不计数"),
        FieldSpec("SEARCH_BREAKER_COOLDOWN_MINUTES", "int", "熔断冷却(分钟)",
                  minimum=0, maximum=1440,
                  hint="0=只记录不跳过；到期自动恢复并重新给一次完整机会"),
