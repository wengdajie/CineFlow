...
| 脚本 | 用途 |
|---|---|
| `smoke_test.py` | 对运行中的实例逐个调用全部 255 项接口用例（含鉴权拦截、自定义站点、追新雷达、热度排行、定时任务改期/重置、插件启停与动作、**网盘总览/浏览/转存**、**ChatOps 指令执行与 Webhook 验签**、**STRM 同步与 302 播放**、**网盘分享追更巡检**、**NFO 补刮与洗版试算**、**在线改配置与恢复默认**、**用户 CRUD 与三档角色 403 边界**、**站点健康探测**、**榜单订阅规则与试算**、**过滤规则组分层试算**、**15 组设置分组**、**搜索每站诊断**、**yt-dlp 付费墙拦截**、**网盘能力位与文件管理全链路**、**网盘凭据保活**、**豆瓣封面兜底**、**图片代理 SSRF 拒绝**、**发现榜五分类与 B 站分区**、**网盘登录能力声明**、**夸克扫码应被拒**、**Cookie 校验不过不写库**），全部通过才退出 0 |
